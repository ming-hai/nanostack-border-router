...
void thread_br_conn_handler_thread_connection_update(bool status)
{
    ...
    tr_debug("mesh0 connected: %d", status);
...
void thread_br_conn_handler_ethernet_connection_update(bool status)
{
    ...
    tr_debug("Eth0 connected: %d", status);
...
//#define TEST_MDNS_IN_MESH
#ifdef TEST_MDNS_IN_MESH
        // For test purposes, send mDNS messages to Thread network
        thread_border_router_mdns_responder_start(thread_br_handler.thread_interface_id, thread_br_handler.thread_interface_id, "ARM-BR");
#else
